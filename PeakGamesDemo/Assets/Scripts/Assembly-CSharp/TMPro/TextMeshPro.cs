...
namespace TMPro
{
	...
	public class TextMeshPro : TMP_Text, ILayoutElement
	{
		...
		private MeshFilter BMGKHDKFAMF;

		private bool KCNOKPCMJBI;

		private int BDEKJLMKCGK = 8;

		private int NEJKKHIGLOP = 4;

		private Bounds ELLAHMGOAPM = new Bounds(Vector3.zero, new Vector3(1000f, 1000f, 0f));
		...
		private bool BMDGJNILCNH;

		[SerializeField]
		private DFBBFGFKCOK m_maskType;
		...
		public int IKJGCDCDCCJ
		{
			...
			set
			{
				...
				if (m_subTextObjects == null || m_subTextObjects.Length == 0)
				{
					return;
				}
				int i = 0;
				for (int num = m_subTextObjects.Length; i < num; i++)
				{
					if (m_subTextObjects[i] != null)
					{
						m_subTextObjects[i].SetSortingLayer(value);
					}
				}
			}
		}

		public int GDDGFAKOCIL
		{
			get
			{
			...
			set
			{
				...
				for (int num = m_subTextObjects.Length; i < num; i++)
				{
					if (m_subTextObjects[i] != null)
					{
						m_subTextObjects[i].SetSortingOrder(value);
					}
				}
			}
		}
		...
		public TextContainer FDEJINLJHIO
		{
			get
			{
				return null;
		...
		public new Transform ANLKLKAINEO
		{
			get
			{
				if (HGNPANDBHBA == null)
		...
		public MeshFilter BOHOPOEDLMA
		{
			get
			{
				if (BMGKHDKFAMF == null)
				{
					BMGKHDKFAMF = GetComponent<MeshFilter>();
				}
				return BMGKHDKFAMF;
			}
		}

		public DFBBFGFKCOK BELAEGJHBBD
		{
			get
			{
				return m_maskType;
			...
			{
				m_maskType = value;
				GPJPKLJBMOB(m_maskType);
			}
		}

		public void SetMask(DFBBFGFKCOK NBFBPNNEKMN, Vector4 GLFCNDIMGDP)
		{
			GPJPKLJBMOB(NBFBPNNEKMN);
			ANBHJFMGGFH(GLFCNDIMGDP);
		}

		public void SetMask(DFBBFGFKCOK NBFBPNNEKMN, Vector4 GLFCNDIMGDP, float EGPHHEGOCJN, float NBAHIMLOMAN)
		{
			GPJPKLJBMOB(NBFBPNNEKMN);
			ANBHJFMGGFH(GLFCNDIMGDP, EGPHHEGOCJN, NBAHIMLOMAN);
		}

		public override void SetVerticesDirty()
		{
			if (!DDIENOMJOHI && !(this == null) && IsActive())
			{
				AKPCCCAJCLD.EHNJMDLGJIP(this);
				DDIENOMJOHI = true;
			}
		}
		...
		public override void ClearMesh(bool AHJKJHGIJIA)
		{
			if (m_textInfo.meshInfo[0].IJKCGGPJDIG == null)
			{
				m_textInfo.meshInfo[0].IJKCGGPJDIG = OPMBPPBMINF;
			}
			m_textInfo.OOPEFLFPKFO(AHJKJHGIJIA);
		}
		...
		public void CalculateLayoutInputHorizontal()
		{
			...
			HDDNJGLELED = m_enableAutoSizing;
			if (DHOELNIONFE || HBKNOEILBLE.hasChanged)
			{
				...
				IOOMPCBBCNK = TMP_Text.FEBIFNLEHEG;
				NNFECNKNNLL = TMP_Text.FEBIFNLEHEG;
		...
		public void CalculateLayoutInputVertical()
		{
			...
			if (DHOELNIONFE || HBKNOEILBLE.hasChanged)
			{
				...
				NNFECNKNNLL = TMP_Text.FEBIFNLEHEG;
				DLIBNLFHMKF();
				m_enableAutoSizing = HDDNJGLELED;
				LNKILMNOLPD = HJNAPNHAJCI.Render;
				EMKNNKLEEPI();
				GHPJHKPEOIK = true;
			}
		...
		protected override void Awake()
		{
			...
			HGNPANDBHBA = ANLKLKAINEO;
			BMGKHDKFAMF = GetComponent<MeshFilter>();
			if (BMGKHDKFAMF == null)
			{
				BMGKHDKFAMF = base.gameObject.AddComponent<MeshFilter>();
			}
			if (OPMBPPBMINF == null)
			{
				OPMBPPBMINF = new Mesh();
				OPMBPPBMINF.hideFlags = HideFlags.HideAndDontSave;
				BMGKHDKFAMF.mesh = OPMBPPBMINF;
			}
			BMGKHDKFAMF.hideFlags = HideFlags.HideInInspector;
			BFCDKEKLEJF();
			HKPPGOIIJJJ();
			...
			if (m_fontAsset == null)
			{
				Debug.LogWarning("Please assign a Font Asset to this " + ANLKLKAINEO.name + " gameobject.", this);
				return;
			}
		...
		{
			AKPCCCAJCLD.CHHGMIDGPKI(this);
			BMGKHDKFAMF.sharedMesh = null;
			HPDBJNLMBED(false);
		}
		...
		protected override void HKPPGOIIJJJ()
		{
			...
			else
			{
				...
				if (m_renderer.sharedMaterial == null || m_renderer.sharedMaterial.GetTexture(MOHPNOBPBBL.GKMMOOGJLHP) == null || m_fontAsset.atlas.GetInstanceID() != m_renderer.sharedMaterial.GetTexture(MOHPNOBPBBL.GKMMOOGJLHP).GetInstanceID())
				{
					m_renderer.sharedMaterial = m_fontAsset.material;
					m_sharedMaterial = m_fontAsset.material;
				}
		...
		private void GPJPKLJBMOB(DFBBFGFKCOK BELAEGJHBBD)
		{
			switch (BELAEGJHBBD)
			{
			case DFBBFGFKCOK.MaskOff:
				m_sharedMaterial.DisableKeyword(MOHPNOBPBBL.OINGPAFNGAP);
				m_sharedMaterial.DisableKeyword(MOHPNOBPBBL.GGAODFHBIKC);
				m_sharedMaterial.DisableKeyword(MOHPNOBPBBL.CNHIJMOJIDL);
				break;
			case DFBBFGFKCOK.MaskSoft:
				m_sharedMaterial.EnableKeyword(MOHPNOBPBBL.OINGPAFNGAP);
				m_sharedMaterial.DisableKeyword(MOHPNOBPBBL.GGAODFHBIKC);
				m_sharedMaterial.DisableKeyword(MOHPNOBPBBL.CNHIJMOJIDL);
				break;
			case DFBBFGFKCOK.MaskHard:
				m_sharedMaterial.EnableKeyword(MOHPNOBPBBL.GGAODFHBIKC);
				m_sharedMaterial.DisableKeyword(MOHPNOBPBBL.OINGPAFNGAP);
				m_sharedMaterial.DisableKeyword(MOHPNOBPBBL.CNHIJMOJIDL);
				break;
			}
		...
		private void ANBHJFMGGFH(Vector4 BFHPJBHCANL)
		{
			m_sharedMaterial.SetVector(MOHPNOBPBBL.EFFCKFEGLNH, BFHPJBHCANL);
		}

		private void ANBHJFMGGFH(Vector4 BFHPJBHCANL, float DABOMJNDMCI, float MFKDCFOONLG)
		{
			m_sharedMaterial.SetVector(MOHPNOBPBBL.EFFCKFEGLNH, BFHPJBHCANL);
			m_sharedMaterial.SetFloat(MOHPNOBPBBL.JPMMJDNBAEK, DABOMJNDMCI);
			m_sharedMaterial.SetFloat(MOHPNOBPBBL.EMNEGMNACHG, MFKDCFOONLG);
		}
		...
		{
			if (HFGCJAKMLKE && HFGCJAKMLKE && m_fontMaterial == null)
			{
				IEDEFFAONFN();
			}
		}
		...
		protected override Material[] EJLJPNINLLO(Material[] BIPEMPFPCKG)
		{
			...
			for (int i = 0; i < materialCount; i++)
			{
				...
				{
					m_fontMaterials[i] = m_subTextObjects[i].GHPFOPNNJOB;
				}
			}
		...
		protected override void MMOPIMDKJDE(Material[] IDBMABBJCNC)
		{
			...
			for (int i = 0; i < materialCount; i++)
			{
				Texture texture = IDBMABBJCNC[i].GetTexture(MOHPNOBPBBL.GKMMOOGJLHP);
				if (i == 0)
				{
					if (!(texture == null) && texture.GetInstanceID() == m_sharedMaterial.GetTexture(MOHPNOBPBBL.GKMMOOGJLHP).GetInstanceID())
					{
						m_sharedMaterial = (m_fontSharedMaterials[i] = IDBMABBJCNC[i]);
						IBKCGEGKGNM = LEJDCNIKNBD(m_sharedMaterial);
					}
				}
				else if (!(texture == null) && texture.GetInstanceID() == m_subTextObjects[i].OOMNMKHLJIC.GetTexture(MOHPNOBPBBL.GKMMOOGJLHP).GetInstanceID() && m_subTextObjects[i].PDLHEANIELH)
				{
		...
		protected override void MIOIBPAPDAN(float GGANGHBLIPC)
		{
			...
			m_renderer.material.SetFloat(MOHPNOBPBBL.KOJOEHKKDCC, GGANGHBLIPC);
		...
		{
			m_renderer.material.SetColor(MOHPNOBPBBL.JCABCKFODAG, MFODILBOMGD);
			if (m_fontMaterial == null)
			{
				m_fontMaterial = m_renderer.material;
			}
		...
		{
			m_renderer.material.SetColor(MOHPNOBPBBL.LNBDEEHONJE, MFODILBOMGD);
			if (m_fontMaterial == null)
			{
				m_fontMaterial = m_renderer.material;
			}
		...
		protected override int JAADOMGCOHA(int[] FBPAOINOCHI)
		{
			...
			for (int j = 0; j < num3; j++)
			{
				if (j > 0)
				{
					...
					{
						m_subTextObjects[j] = TMP_SubMesh.AddSubTextObject(this, NDPGMAPHNNA[j]);
				...
				if (m_textInfo.meshInfo[j].FHBFCAEGPOH == null || m_textInfo.meshInfo[j].FHBFCAEGPOH.Length < oLEEIKHIDIB * (m_isVolumetricText ? 8 : 4))
				{
					if (m_textInfo.meshInfo[j].FHBFCAEGPOH == null)
					{
						if (j == 0)
						{
							m_textInfo.meshInfo[j] = new IDAPNGDHHBD(OPMBPPBMINF, oLEEIKHIDIB + 1, m_isVolumetricText);
						}
						else
						{
							m_textInfo.meshInfo[j] = new IDAPNGDHHBD(m_subTextObjects[j].IJKCGGPJDIG, oLEEIKHIDIB + 1, m_isVolumetricText);
						}
					}
					else
					{
						m_textInfo.meshInfo[j].CMGCBJCIJNB((oLEEIKHIDIB <= 1024) ? Mathf.NextPowerOfTwo(oLEEIKHIDIB) : (oLEEIKHIDIB + 256), m_isVolumetricText);
					}
				}
				else if (m_textInfo.meshInfo[j].FHBFCAEGPOH.Length - oLEEIKHIDIB * (m_isVolumetricText ? 8 : 4) > 1024)
				{
					m_textInfo.meshInfo[j].CMGCBJCIJNB((oLEEIKHIDIB <= 1024) ? Mathf.Max(Mathf.NextPowerOfTwo(oLEEIKHIDIB), 256) : (oLEEIKHIDIB + 256), m_isVolumetricText);
				}
			}
			for (int k = num3; k < m_subTextObjects.Length && m_subTextObjects[k] != null; k++)
			{
				...
				{
					m_textInfo.meshInfo[k].NGOKLPFCFAO(0, true);
				}
			}
			return KOIMFMFOLII;
		}
		...
		protected override void DLIBNLFHMKF()
		{
			...
			MEPDJFGMGOK = m_fontSize / AFEJPPDMPHE.fontInfo.PointSize * ((!m_isOrthographic) ? 0.1f : 1f);
			...
			for (int i = 0; i < GMALEIECAPN.Length && GMALEIECAPN[i] != 0; i++)
			{
				...
				if (FHJFKDGNODI == HIPDBPNMONO.Sprite)
				{
					...
					AFEJPPDMPHE = m_fontAsset;
					float num18 = MPIGOGEKIPI / m_fontAsset.fontInfo.PointSize * m_fontAsset.fontInfo.Scale * ((!m_isOrthographic) ? 0.1f : 1f);
				...
				else if (FHJFKDGNODI == HIPDBPNMONO.Character)
				{
					...
					AIJEDOAJAPI = m_textInfo.characterInfo[HEEDOMEABKA].EKBHGEBCJHL;
					MEPDJFGMGOK = MPIGOGEKIPI * num17 / AFEJPPDMPHE.fontInfo.PointSize * AFEJPPDMPHE.fontInfo.Scale * ((!m_isOrthographic) ? 0.1f : 1f);
			...
			int EHJNMODJEAN = NDPGMAPHNNA[0].OLEEIKHIDIB * (m_isVolumetricText ? 8 : 4);
			...
			int num40 = 0;
			int num41 = 0;
			float num42 = (JFGEKNMANOJ = ANLKLKAINEO.lossyScale.y);
			...
			for (int j = 0; j < HEEDOMEABKA; j++)
			{
				...
				case KNOECCJHIDP.CaplineFlush:
				{
					...
					bool flag12 = (jOPPDFNGJBM & (KNOECCJHIDP)16) == (KNOECCJHIDP)16;
					if ((!char.IsControl(aONJCHFEHOM3) && hGCMIDFMKHD < FGMGAJAPPJC) || flag12 || cFLMKMOJIJI.HDDCEFKKDHE > cFLMKMOJIJI.HHHMOGOKFHF)
					{
				...
				if (bACHBACHNOF)
				{
					...
					case HIPDBPNMONO.Character:
					{
						...
						num43 = characterInfo[j].HJLOCAEAFMJ * num42 * (1f - JDKKGHCOCJK);
					...
					{
					case HIPDBPNMONO.Character:
						ENEAMABFPLB(j, kBAFACHPHMF, m_isVolumetricText);
						break;
				...
				bool flag14 = (m_textInfo.characterInfo[j].LMNINFLOCIB & BCIEOOJOMDO.Strikethrough) == BCIEOOJOMDO.Strikethrough;
				...
				if (flag14)
				{
					...
					if (!flag2 && flag15 && j <= cFLMKMOJIJI.IKKAPEMFJBC && aONJCHFEHOM2 != '\n' && aONJCHFEHOM2 != '\r' && (j != cFLMKMOJIJI.IKKAPEMFJBC || !char.IsSeparator(aONJCHFEHOM2)))
					{
						flag2 = true;
				...
				if ((m_textInfo.characterInfo[j].LMNINFLOCIB & BCIEOOJOMDO.Highlight) == BCIEOOJOMDO.Highlight)
				{
					...
					if (!flag3 && flag16 && j <= cFLMKMOJIJI.IKKAPEMFJBC && aONJCHFEHOM2 != '\n' && aONJCHFEHOM2 != '\r' && (j != cFLMKMOJIJI.IKKAPEMFJBC || !char.IsSeparator(aONJCHFEHOM2)))
					{
						flag3 = true;
		...
		private void JFLNLAPEGII(bool BGEDKGLJIGM)
		{
			if (BMGKHDKFAMF != null)
			{
				if (BGEDKGLJIGM)
				{
					BMGKHDKFAMF.sharedMesh = OPMBPPBMINF;
				}
				else
				{
					BMGKHDKFAMF.sharedMesh = null;
			...
			{
				if (m_subTextObjects[i].BOHOPOEDLMA != null)
				{
					if (BGEDKGLJIGM)
					{
						m_subTextObjects[i].BOHOPOEDLMA.sharedMesh = m_subTextObjects[i].IJKCGGPJDIG;
					}
					else
					{
						m_subTextObjects[i].BOHOPOEDLMA.sharedMesh = null;
					}
				}
			}
		}
		...
		protected override void EACFEBIIMJF()
		{
			for (int i = 1; i < m_subTextObjects.Length && m_subTextObjects[i] != null; i++)
			{
		...
		private void GFLNMPPCJKM(float FCCMJOKDDEA)
		{
			for (int i = 0; i < m_textInfo.characterCount; i++)
			{
				...
				{
					float num = FCCMJOKDDEA * m_textInfo.characterInfo[i].HJLOCAEAFMJ * (1f - JDKKGHCOCJK);
					...
					m_textInfo.meshInfo[eKBHGEBCJHL].FJPNPHPFBFJ[kAKJKCHHBAP].y = num;
					m_textInfo.meshInfo[eKBHGEBCJHL].FJPNPHPFBFJ[kAKJKCHHBAP + 1].y = num;
					m_textInfo.meshInfo[eKBHGEBCJHL].FJPNPHPFBFJ[kAKJKCHHBAP + 2].y = num;
					m_textInfo.meshInfo[eKBHGEBCJHL].FJPNPHPFBFJ[kAKJKCHHBAP + 3].y = num;
				}
			}
