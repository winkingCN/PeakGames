...
public class EONJLOEDDIL : JMPFLJGHAIM
{
	private bool HJBGFGMEOOD;

	private bool MGEHJDECCEL;

	private const int ANKLOALOPKP = 16;

	private uint[] PJLEGLOPHFJ = new uint[96];

	private uint[] MCHDDAKFCIC = new uint[8];

	private uint[] EPOECGIAEHH = new uint[12];

	private uint[] BGEDKGLJIGM = new uint[4];

	private static readonly uint[] AFFNIPFAFAJ = new uint[12]
	{
	...
	private static readonly uint[] BPCGACJEDKG = new uint[256]
	{
	...
	private static readonly uint[] JPMMEJLBCNO = new uint[256]
	{
	...
	private static readonly uint[] EENAMBKEIHL = new uint[256]
	{
	...
	private static readonly uint[] FCOOOPIEIOG = new uint[256]
	{
	...
	private static uint EJLFLEENBJE(uint AMFCENFNAJJ, int IBBBMACIBGK)
	{
		return (AMFCENFNAJJ >> IBBBMACIBGK) + (AMFCENFNAJJ << 32 - IBBBMACIBGK);
	}

	private static uint OPPHKJHIDIL(uint AMFCENFNAJJ, int IBBBMACIBGK)
	{
		return (AMFCENFNAJJ << IBBBMACIBGK) + (AMFCENFNAJJ >> 32 - IBBBMACIBGK);
	}

	private static void POLFHOOBANH(int JMFJBAEAFHB, uint[] POOAFGKCLHH, int PPJKJPDABCM, uint[] KHABNLAHHMF, int CNENFBKCLEB)
	{
		KHABNLAHHMF[CNENFBKCLEB] = (POOAFGKCLHH[PPJKJPDABCM] << JMFJBAEAFHB) | (POOAFGKCLHH[1 + PPJKJPDABCM] >> 32 - JMFJBAEAFHB);
		KHABNLAHHMF[1 + CNENFBKCLEB] = (POOAFGKCLHH[1 + PPJKJPDABCM] << JMFJBAEAFHB) | (POOAFGKCLHH[2 + PPJKJPDABCM] >> 32 - JMFJBAEAFHB);
		KHABNLAHHMF[2 + CNENFBKCLEB] = (POOAFGKCLHH[2 + PPJKJPDABCM] << JMFJBAEAFHB) | (POOAFGKCLHH[3 + PPJKJPDABCM] >> 32 - JMFJBAEAFHB);
		KHABNLAHHMF[3 + CNENFBKCLEB] = (POOAFGKCLHH[3 + PPJKJPDABCM] << JMFJBAEAFHB) | (POOAFGKCLHH[PPJKJPDABCM] >> 32 - JMFJBAEAFHB);
		POOAFGKCLHH[PPJKJPDABCM] = KHABNLAHHMF[CNENFBKCLEB];
		POOAFGKCLHH[1 + PPJKJPDABCM] = KHABNLAHHMF[1 + CNENFBKCLEB];
		POOAFGKCLHH[2 + PPJKJPDABCM] = KHABNLAHHMF[2 + CNENFBKCLEB];
		POOAFGKCLHH[3 + PPJKJPDABCM] = KHABNLAHHMF[3 + CNENFBKCLEB];
	}

	private static void OBALHHAAHOB(int JMFJBAEAFHB, uint[] POOAFGKCLHH, int PPJKJPDABCM, uint[] KHABNLAHHMF, int CNENFBKCLEB)
	{
		KHABNLAHHMF[2 + CNENFBKCLEB] = (POOAFGKCLHH[PPJKJPDABCM] << JMFJBAEAFHB) | (POOAFGKCLHH[1 + PPJKJPDABCM] >> 32 - JMFJBAEAFHB);
		KHABNLAHHMF[3 + CNENFBKCLEB] = (POOAFGKCLHH[1 + PPJKJPDABCM] << JMFJBAEAFHB) | (POOAFGKCLHH[2 + PPJKJPDABCM] >> 32 - JMFJBAEAFHB);
		KHABNLAHHMF[CNENFBKCLEB] = (POOAFGKCLHH[2 + PPJKJPDABCM] << JMFJBAEAFHB) | (POOAFGKCLHH[3 + PPJKJPDABCM] >> 32 - JMFJBAEAFHB);
		KHABNLAHHMF[1 + CNENFBKCLEB] = (POOAFGKCLHH[3 + PPJKJPDABCM] << JMFJBAEAFHB) | (POOAFGKCLHH[PPJKJPDABCM] >> 32 - JMFJBAEAFHB);
		POOAFGKCLHH[PPJKJPDABCM] = KHABNLAHHMF[2 + CNENFBKCLEB];
		POOAFGKCLHH[1 + PPJKJPDABCM] = KHABNLAHHMF[3 + CNENFBKCLEB];
		POOAFGKCLHH[2 + PPJKJPDABCM] = KHABNLAHHMF[CNENFBKCLEB];
		POOAFGKCLHH[3 + PPJKJPDABCM] = KHABNLAHHMF[1 + CNENFBKCLEB];
	}

	private static void KEJJINNDJFK(int JMFJBAEAFHB, uint[] POOAFGKCLHH, int PPJKJPDABCM, uint[] KHABNLAHHMF, int CNENFBKCLEB)
	{
		KHABNLAHHMF[CNENFBKCLEB] = (POOAFGKCLHH[1 + PPJKJPDABCM] << JMFJBAEAFHB - 32) | (POOAFGKCLHH[2 + PPJKJPDABCM] >> 64 - JMFJBAEAFHB);
		KHABNLAHHMF[1 + CNENFBKCLEB] = (POOAFGKCLHH[2 + PPJKJPDABCM] << JMFJBAEAFHB - 32) | (POOAFGKCLHH[3 + PPJKJPDABCM] >> 64 - JMFJBAEAFHB);
		KHABNLAHHMF[2 + CNENFBKCLEB] = (POOAFGKCLHH[3 + PPJKJPDABCM] << JMFJBAEAFHB - 32) | (POOAFGKCLHH[PPJKJPDABCM] >> 64 - JMFJBAEAFHB);
		KHABNLAHHMF[3 + CNENFBKCLEB] = (POOAFGKCLHH[PPJKJPDABCM] << JMFJBAEAFHB - 32) | (POOAFGKCLHH[1 + PPJKJPDABCM] >> 64 - JMFJBAEAFHB);
		POOAFGKCLHH[PPJKJPDABCM] = KHABNLAHHMF[CNENFBKCLEB];
		POOAFGKCLHH[1 + PPJKJPDABCM] = KHABNLAHHMF[1 + CNENFBKCLEB];
		POOAFGKCLHH[2 + PPJKJPDABCM] = KHABNLAHHMF[2 + CNENFBKCLEB];
		POOAFGKCLHH[3 + PPJKJPDABCM] = KHABNLAHHMF[3 + CNENFBKCLEB];
	...
	private static void MEMNIDNIDAK(int JMFJBAEAFHB, uint[] POOAFGKCLHH, int PPJKJPDABCM, uint[] KHABNLAHHMF, int CNENFBKCLEB)
	{
		KHABNLAHHMF[2 + CNENFBKCLEB] = (POOAFGKCLHH[1 + PPJKJPDABCM] << JMFJBAEAFHB - 32) | (POOAFGKCLHH[2 + PPJKJPDABCM] >> 64 - JMFJBAEAFHB);
		KHABNLAHHMF[3 + CNENFBKCLEB] = (POOAFGKCLHH[2 + PPJKJPDABCM] << JMFJBAEAFHB - 32) | (POOAFGKCLHH[3 + PPJKJPDABCM] >> 64 - JMFJBAEAFHB);
		KHABNLAHHMF[CNENFBKCLEB] = (POOAFGKCLHH[3 + PPJKJPDABCM] << JMFJBAEAFHB - 32) | (POOAFGKCLHH[PPJKJPDABCM] >> 64 - JMFJBAEAFHB);
		KHABNLAHHMF[1 + CNENFBKCLEB] = (POOAFGKCLHH[PPJKJPDABCM] << JMFJBAEAFHB - 32) | (POOAFGKCLHH[1 + PPJKJPDABCM] >> 64 - JMFJBAEAFHB);
		POOAFGKCLHH[PPJKJPDABCM] = KHABNLAHHMF[2 + CNENFBKCLEB];
		POOAFGKCLHH[1 + PPJKJPDABCM] = KHABNLAHHMF[3 + CNENFBKCLEB];
		POOAFGKCLHH[2 + PPJKJPDABCM] = KHABNLAHHMF[CNENFBKCLEB];
		POOAFGKCLHH[3 + PPJKJPDABCM] = KHABNLAHHMF[1 + CNENFBKCLEB];
	...
	private static uint LPIABPJJLAK(byte[] ENICBMKPJJK, int IBNDGNOIEEF)
	{
		uint num = 0u;
		for (int i = 0; i < 4; i++)
		{
			num = (num << 8) + ENICBMKPJJK[i + IBNDGNOIEEF];
		}
		return num;
	}

	private static void OBCCDKDJLPH(uint BKKMJAIKJAL, byte[] PPFHFMDDDLM, int IBNDGNOIEEF)
	{
		for (int i = 0; i < 4; i++)
		{
			PPFHFMDDDLM[3 - i + IBNDGNOIEEF] = (byte)BKKMJAIKJAL;
			BKKMJAIKJAL >>= 8;
		}
	}

	private static void JKJNCHCHHMK(uint[] IBBBMACIBGK, uint[] NLAAEBMELBD, int NPFGAPHPEGP)
	{
		uint num = IBBBMACIBGK[0] ^ NLAAEBMELBD[NPFGAPHPEGP];
		uint num2 = JPMMEJLBCNO[(byte)num];
		num2 ^= FCOOOPIEIOG[(byte)(num >> 8)];
		num2 ^= EENAMBKEIHL[(byte)(num >> 16)];
		num2 ^= BPCGACJEDKG[(byte)(num >> 24)];
		uint num3 = IBBBMACIBGK[1] ^ NLAAEBMELBD[1 + NPFGAPHPEGP];
		uint num4 = BPCGACJEDKG[(byte)num3];
		num4 ^= JPMMEJLBCNO[(byte)(num3 >> 8)];
		num4 ^= FCOOOPIEIOG[(byte)(num3 >> 16)];
		num4 ^= EENAMBKEIHL[(byte)(num3 >> 24)];
		IBBBMACIBGK[2] ^= num2 ^ num4;
		IBBBMACIBGK[3] ^= num2 ^ num4 ^ EJLFLEENBJE(num2, 8);
		num = IBBBMACIBGK[2] ^ NLAAEBMELBD[2 + NPFGAPHPEGP];
		num2 = JPMMEJLBCNO[(byte)num];
		num2 ^= FCOOOPIEIOG[(byte)(num >> 8)];
		num2 ^= EENAMBKEIHL[(byte)(num >> 16)];
		num2 ^= BPCGACJEDKG[(byte)(num >> 24)];
		num3 = IBBBMACIBGK[3] ^ NLAAEBMELBD[3 + NPFGAPHPEGP];
		num4 = BPCGACJEDKG[(byte)num3];
		num4 ^= JPMMEJLBCNO[(byte)(num3 >> 8)];
		num4 ^= FCOOOPIEIOG[(byte)(num3 >> 16)];
		num4 ^= EENAMBKEIHL[(byte)(num3 >> 24)];
		IBBBMACIBGK[0] ^= num2 ^ num4;
		IBBBMACIBGK[1] ^= num2 ^ num4 ^ EJLFLEENBJE(num2, 8);
	}

	private static void GKKBICAMFNA(uint[] IBBBMACIBGK, uint[] GANAJDJFPPJ, int NPFGAPHPEGP)
	{
		IBBBMACIBGK[1] ^= OPPHKJHIDIL(IBBBMACIBGK[0] & GANAJDJFPPJ[NPFGAPHPEGP], 1);
		IBBBMACIBGK[0] ^= GANAJDJFPPJ[1 + NPFGAPHPEGP] | IBBBMACIBGK[1];
		IBBBMACIBGK[2] ^= GANAJDJFPPJ[3 + NPFGAPHPEGP] | IBBBMACIBGK[3];
		IBBBMACIBGK[3] ^= OPPHKJHIDIL(GANAJDJFPPJ[2 + NPFGAPHPEGP] & IBBBMACIBGK[2], 1);
	}

	private void LHCPMMKLJGA(bool MAEALKEJALK, byte[] IKNCPEPOKGJ)
	{
		uint[] array = new uint[8];
		uint[] array2 = new uint[4];
		uint[] array3 = new uint[4];
		uint[] array4 = new uint[4];
		switch (IKNCPEPOKGJ.Length)
		{
		case 16:
			MGEHJDECCEL = true;
			array[0] = LPIABPJJLAK(IKNCPEPOKGJ, 0);
			array[1] = LPIABPJJLAK(IKNCPEPOKGJ, 4);
			array[2] = LPIABPJJLAK(IKNCPEPOKGJ, 8);
			array[3] = LPIABPJJLAK(IKNCPEPOKGJ, 12);
		...
		case 24:
			...
			array[2] = LPIABPJJLAK(IKNCPEPOKGJ, 8);
			array[3] = LPIABPJJLAK(IKNCPEPOKGJ, 12);
			array[4] = LPIABPJJLAK(IKNCPEPOKGJ, 16);
			array[5] = LPIABPJJLAK(IKNCPEPOKGJ, 20);
			array[6] = ~array[4];
			array[7] = ~array[5];
			MGEHJDECCEL = false;
			break;
		case 32:
			...
			array[4] = LPIABPJJLAK(IKNCPEPOKGJ, 16);
			array[5] = LPIABPJJLAK(IKNCPEPOKGJ, 20);
			array[6] = LPIABPJJLAK(IKNCPEPOKGJ, 24);
			array[7] = LPIABPJJLAK(IKNCPEPOKGJ, 28);
			MGEHJDECCEL = false;
			break;
		default:
		...
		JKJNCHCHHMK(array2, AFFNIPFAFAJ, 0);
		for (int j = 0; j < 4; j++)
		{
			array2[j] ^= array[j];
		}
		JKJNCHCHHMK(array2, AFFNIPFAFAJ, 4);
		if (MGEHJDECCEL)
		{
			if (MAEALKEJALK)
			{
				MCHDDAKFCIC[0] = array[0];
				MCHDDAKFCIC[1] = array[1];
				MCHDDAKFCIC[2] = array[2];
				MCHDDAKFCIC[3] = array[3];
				POLFHOOBANH(15, array, 0, PJLEGLOPHFJ, 4);
				POLFHOOBANH(30, array, 0, PJLEGLOPHFJ, 12);
				POLFHOOBANH(15, array, 0, array4, 0);
				PJLEGLOPHFJ[18] = array4[2];
				PJLEGLOPHFJ[19] = array4[3];
				POLFHOOBANH(17, array, 0, EPOECGIAEHH, 4);
				POLFHOOBANH(17, array, 0, PJLEGLOPHFJ, 24);
				POLFHOOBANH(17, array, 0, PJLEGLOPHFJ, 32);
				PJLEGLOPHFJ[0] = array2[0];
				PJLEGLOPHFJ[1] = array2[1];
				PJLEGLOPHFJ[2] = array2[2];
				PJLEGLOPHFJ[3] = array2[3];
				POLFHOOBANH(15, array2, 0, PJLEGLOPHFJ, 8);
				POLFHOOBANH(15, array2, 0, EPOECGIAEHH, 0);
				POLFHOOBANH(15, array2, 0, array4, 0);
				PJLEGLOPHFJ[16] = array4[0];
				PJLEGLOPHFJ[17] = array4[1];
				POLFHOOBANH(15, array2, 0, PJLEGLOPHFJ, 20);
				KEJJINNDJFK(34, array2, 0, PJLEGLOPHFJ, 28);
				POLFHOOBANH(17, array2, 0, MCHDDAKFCIC, 4);
			}
			else
			{
				MCHDDAKFCIC[4] = array[0];
				MCHDDAKFCIC[5] = array[1];
				MCHDDAKFCIC[6] = array[2];
				MCHDDAKFCIC[7] = array[3];
				OBALHHAAHOB(15, array, 0, PJLEGLOPHFJ, 28);
				OBALHHAAHOB(30, array, 0, PJLEGLOPHFJ, 20);
				OBALHHAAHOB(15, array, 0, array4, 0);
				PJLEGLOPHFJ[16] = array4[0];
				PJLEGLOPHFJ[17] = array4[1];
				OBALHHAAHOB(17, array, 0, EPOECGIAEHH, 0);
				OBALHHAAHOB(17, array, 0, PJLEGLOPHFJ, 8);
				OBALHHAAHOB(17, array, 0, PJLEGLOPHFJ, 0);
				PJLEGLOPHFJ[34] = array2[0];
				PJLEGLOPHFJ[35] = array2[1];
				PJLEGLOPHFJ[32] = array2[2];
				PJLEGLOPHFJ[33] = array2[3];
				OBALHHAAHOB(15, array2, 0, PJLEGLOPHFJ, 24);
				OBALHHAAHOB(15, array2, 0, EPOECGIAEHH, 4);
				OBALHHAAHOB(15, array2, 0, array4, 0);
				PJLEGLOPHFJ[18] = array4[2];
				PJLEGLOPHFJ[19] = array4[3];
				OBALHHAAHOB(15, array2, 0, PJLEGLOPHFJ, 12);
				MEMNIDNIDAK(34, array2, 0, PJLEGLOPHFJ, 4);
				POLFHOOBANH(17, array2, 0, MCHDDAKFCIC, 0);
			}
			return;
		}
		for (int k = 0; k < 4; k++)
		...
		JKJNCHCHHMK(array3, AFFNIPFAFAJ, 8);
		if (MAEALKEJALK)
		{
			MCHDDAKFCIC[0] = array[0];
			MCHDDAKFCIC[1] = array[1];
			MCHDDAKFCIC[2] = array[2];
			MCHDDAKFCIC[3] = array[3];
			KEJJINNDJFK(45, array, 0, PJLEGLOPHFJ, 16);
			POLFHOOBANH(15, array, 0, EPOECGIAEHH, 4);
			POLFHOOBANH(17, array, 0, PJLEGLOPHFJ, 32);
			KEJJINNDJFK(34, array, 0, PJLEGLOPHFJ, 44);
			POLFHOOBANH(15, array, 4, PJLEGLOPHFJ, 4);
			POLFHOOBANH(15, array, 4, EPOECGIAEHH, 0);
			POLFHOOBANH(30, array, 4, PJLEGLOPHFJ, 24);
			KEJJINNDJFK(34, array, 4, PJLEGLOPHFJ, 36);
			POLFHOOBANH(15, array2, 0, PJLEGLOPHFJ, 8);
			POLFHOOBANH(30, array2, 0, PJLEGLOPHFJ, 20);
			EPOECGIAEHH[8] = array2[1];
			EPOECGIAEHH[9] = array2[2];
			EPOECGIAEHH[10] = array2[3];
			EPOECGIAEHH[11] = array2[0];
			KEJJINNDJFK(49, array2, 0, PJLEGLOPHFJ, 40);
			PJLEGLOPHFJ[0] = array3[0];
			PJLEGLOPHFJ[1] = array3[1];
			PJLEGLOPHFJ[2] = array3[2];
			PJLEGLOPHFJ[3] = array3[3];
			POLFHOOBANH(30, array3, 0, PJLEGLOPHFJ, 12);
			POLFHOOBANH(30, array3, 0, PJLEGLOPHFJ, 28);
			KEJJINNDJFK(51, array3, 0, MCHDDAKFCIC, 4);
		}
		else
		{
			MCHDDAKFCIC[4] = array[0];
			...
			MCHDDAKFCIC[6] = array[2];
			MCHDDAKFCIC[7] = array[3];
			MEMNIDNIDAK(45, array, 0, PJLEGLOPHFJ, 28);
			OBALHHAAHOB(15, array, 0, EPOECGIAEHH, 4);
			OBALHHAAHOB(17, array, 0, PJLEGLOPHFJ, 12);
			MEMNIDNIDAK(34, array, 0, PJLEGLOPHFJ, 0);
			OBALHHAAHOB(15, array, 4, PJLEGLOPHFJ, 40);
			OBALHHAAHOB(15, array, 4, EPOECGIAEHH, 8);
			OBALHHAAHOB(30, array, 4, PJLEGLOPHFJ, 20);
			MEMNIDNIDAK(34, array, 4, PJLEGLOPHFJ, 8);
			OBALHHAAHOB(15, array2, 0, PJLEGLOPHFJ, 36);
			OBALHHAAHOB(30, array2, 0, PJLEGLOPHFJ, 24);
			EPOECGIAEHH[2] = array2[1];
			EPOECGIAEHH[3] = array2[2];
			EPOECGIAEHH[0] = array2[3];
			EPOECGIAEHH[1] = array2[0];
			MEMNIDNIDAK(49, array2, 0, PJLEGLOPHFJ, 4);
			PJLEGLOPHFJ[46] = array3[0];
			PJLEGLOPHFJ[47] = array3[1];
			PJLEGLOPHFJ[44] = array3[2];
			PJLEGLOPHFJ[45] = array3[3];
			OBALHHAAHOB(30, array3, 0, PJLEGLOPHFJ, 32);
			OBALHHAAHOB(30, array3, 0, PJLEGLOPHFJ, 16);
			KEJJINNDJFK(51, array3, 0, MCHDDAKFCIC, 0);
		}
	}

	private int HLOHCJOGHDG(byte[] MOPCLEEGDFB, int ELICNEDIBGB, byte[] BILGCJNPJGJ, int EMAKNGBIFDC)
	{
		for (int i = 0; i < 4; i++)
		{
			BGEDKGLJIGM[i] = LPIABPJJLAK(MOPCLEEGDFB, ELICNEDIBGB + i * 4);
			BGEDKGLJIGM[i] ^= MCHDDAKFCIC[i];
		}
		JKJNCHCHHMK(BGEDKGLJIGM, PJLEGLOPHFJ, 0);
		JKJNCHCHHMK(BGEDKGLJIGM, PJLEGLOPHFJ, 4);
		JKJNCHCHHMK(BGEDKGLJIGM, PJLEGLOPHFJ, 8);
		GKKBICAMFNA(BGEDKGLJIGM, EPOECGIAEHH, 0);
		JKJNCHCHHMK(BGEDKGLJIGM, PJLEGLOPHFJ, 12);
		JKJNCHCHHMK(BGEDKGLJIGM, PJLEGLOPHFJ, 16);
		JKJNCHCHHMK(BGEDKGLJIGM, PJLEGLOPHFJ, 20);
		GKKBICAMFNA(BGEDKGLJIGM, EPOECGIAEHH, 4);
		JKJNCHCHHMK(BGEDKGLJIGM, PJLEGLOPHFJ, 24);
		JKJNCHCHHMK(BGEDKGLJIGM, PJLEGLOPHFJ, 28);
		JKJNCHCHHMK(BGEDKGLJIGM, PJLEGLOPHFJ, 32);
		BGEDKGLJIGM[2] ^= MCHDDAKFCIC[4];
		BGEDKGLJIGM[3] ^= MCHDDAKFCIC[5];
		BGEDKGLJIGM[0] ^= MCHDDAKFCIC[6];
		BGEDKGLJIGM[1] ^= MCHDDAKFCIC[7];
		OBCCDKDJLPH(BGEDKGLJIGM[2], BILGCJNPJGJ, EMAKNGBIFDC);
		OBCCDKDJLPH(BGEDKGLJIGM[3], BILGCJNPJGJ, EMAKNGBIFDC + 4);
		OBCCDKDJLPH(BGEDKGLJIGM[0], BILGCJNPJGJ, EMAKNGBIFDC + 8);
		OBCCDKDJLPH(BGEDKGLJIGM[1], BILGCJNPJGJ, EMAKNGBIFDC + 12);
		return 16;
	}

	private int LFKKGKKIGFB(byte[] MOPCLEEGDFB, int ELICNEDIBGB, byte[] BILGCJNPJGJ, int EMAKNGBIFDC)
	{
		for (int i = 0; i < 4; i++)
		{
			BGEDKGLJIGM[i] = LPIABPJJLAK(MOPCLEEGDFB, ELICNEDIBGB + i * 4);
		...
		JKJNCHCHHMK(BGEDKGLJIGM, PJLEGLOPHFJ, 28);
		JKJNCHCHHMK(BGEDKGLJIGM, PJLEGLOPHFJ, 32);
		GKKBICAMFNA(BGEDKGLJIGM, EPOECGIAEHH, 8);
		JKJNCHCHHMK(BGEDKGLJIGM, PJLEGLOPHFJ, 36);
		JKJNCHCHHMK(BGEDKGLJIGM, PJLEGLOPHFJ, 40);
		JKJNCHCHHMK(BGEDKGLJIGM, PJLEGLOPHFJ, 44);
		BGEDKGLJIGM[2] ^= MCHDDAKFCIC[4];
		BGEDKGLJIGM[3] ^= MCHDDAKFCIC[5];
		BGEDKGLJIGM[0] ^= MCHDDAKFCIC[6];
		BGEDKGLJIGM[1] ^= MCHDDAKFCIC[7];
	...
	public virtual void DIOELAHNLKJ(bool MAEALKEJALK, IGMAJHKIIPK IJJHBEECMBP)
	{
		...
		LHCPMMKLJGA(MAEALKEJALK, ((JEMNCGMEABF)IJJHBEECMBP).MEGBFHFMBFI());
		HJBGFGMEOOD = true;
	}
	...
	public virtual int DPACJDFHLKB(byte[] MOPCLEEGDFB, int ELICNEDIBGB, byte[] BILGCJNPJGJ, int EMAKNGBIFDC)
	{
		...
		if (MGEHJDECCEL)
		{
			return HLOHCJOGHDG(MOPCLEEGDFB, ELICNEDIBGB, BILGCJNPJGJ, EMAKNGBIFDC);
		}
		return LFKKGKKIGFB(MOPCLEEGDFB, ELICNEDIBGB, BILGCJNPJGJ, EMAKNGBIFDC);
	}
