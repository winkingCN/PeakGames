...
public class HDNPEKKELBE : JMPFLJGHAIM
{
	internal const int ANKLOALOPKP = 8;

	private int[] IOANFHFGPJK;

	private static readonly short[] LIDHLCFELFM = new short[8] { 128, 64, 32, 16, 8, 4, 2, 1 };

	private static readonly int[] FFAONHIFEBE = new int[24]
	{
	...
	private static readonly byte[] JLDDJNIDDDF = new byte[56]
	{
	...
	private static readonly byte[] MMFLDJOCOOL = new byte[16]
	{
		1, 2, 4, 6, 8, 10, 12, 14, 15, 17,
		19, 21, 23, 25, 27, 28
	};

	private static readonly byte[] NFPHEDLMLML = new byte[48]
	{
	...
	private static readonly uint[] HLKNLALDKBA = new uint[64]
	{
	...
	private static readonly uint[] EJMIOHKANIA = new uint[64]
	{
	...
	private static readonly uint[] OPHIFECEKJM = new uint[64]
	{
	...
	private static readonly uint[] CLJNBBNMKOC = new uint[64]
	{
	...
	private static readonly uint[] FNOFEPDINLN = new uint[64]
	{
	...
	private static readonly uint[] GAMFPABJNDC = new uint[64]
	{
	...
	private static readonly uint[] ONOJPLONDMO = new uint[64]
	{
	...
	private static readonly uint[] NDKJBHKFPPG = new uint[64]
	{
	...
	public virtual int[] KKDIDGCNCJK()
	{
		return IOANFHFGPJK;
	}
	...
	public virtual void DIOELAHNLKJ(bool MAEALKEJALK, IGMAJHKIIPK IJJHBEECMBP)
	{
		...
		{
			throw new ArgumentException("invalid parameter passed to DES init - " + LHFGHNFJIKM.JKHOEKAOLPM(IJJHBEECMBP));
		}
	...
	public virtual int DPACJDFHLKB(byte[] MOPCLEEGDFB, int ELICNEDIBGB, byte[] BILGCJNPJGJ, int EMAKNGBIFDC)
	{
		...
		JMOHJLGLOBK(IOANFHFGPJK, MOPCLEEGDFB, ELICNEDIBGB, BILGCJNPJGJ, EMAKNGBIFDC);
		return 8;
	}
	...
	protected static int[] DNIOCLEEKMH(bool LFBBFLCMAHI, byte[] IKNCPEPOKGJ)
	{
		...
		for (int i = 0; i < 56; i++)
		{
			int num = JLDDJNIDDDF[i];
			array2[i] = (IKNCPEPOKGJ[(uint)num >> 3] & LIDHLCFELFM[num & 7]) != 0;
		}
		for (int j = 0; j < 16; j++)
		{
			int num2 = ((!LFBBFLCMAHI) ? (15 - j << 1) : (j << 1));
			int num3 = num2 + 1;
			array[num2] = (array[num3] = 0);
			for (int k = 0; k < 28; k++)
			{
				int num4 = k + MMFLDJOCOOL[j];
				if (num4 < 28)
				{
			...
			for (int m = 0; m < 24; m++)
			{
				if (array3[NFPHEDLMLML[m]])
				{
					array[num2] |= FFAONHIFEBE[m];
				}
				if (array3[NFPHEDLMLML[m + 24]])
				{
					array[num3] |= FFAONHIFEBE[m];
				}
			}
		}
	...
	internal static void JMOHJLGLOBK(int[] IIIGKBPIKGE, byte[] MOPCLEEGDFB, int ELICNEDIBGB, byte[] NGAGMIIBGKA, int EMAKNGBIFDC)
	{
		uint num = PGOKFBKEOAB.EFODDMAJNOA(MOPCLEEGDFB, ELICNEDIBGB);
		uint num2 = PGOKFBKEOAB.EFODDMAJNOA(MOPCLEEGDFB, ELICNEDIBGB + 4);
		...
		{
			num3 = (num2 << 28) | (num2 >> 4);
			num3 ^= (uint)IIIGKBPIKGE[i * 4];
			uint num4 = ONOJPLONDMO[num3 & 0x3F];
			num4 |= FNOFEPDINLN[(num3 >> 8) & 0x3F];
			num4 |= OPHIFECEKJM[(num3 >> 16) & 0x3F];
			num4 |= HLKNLALDKBA[(num3 >> 24) & 0x3F];
			num3 = num2 ^ (uint)IIIGKBPIKGE[i * 4 + 1];
			num4 |= NDKJBHKFPPG[num3 & 0x3F];
			num4 |= GAMFPABJNDC[(num3 >> 8) & 0x3F];
			num4 |= CLJNBBNMKOC[(num3 >> 16) & 0x3F];
			num4 |= EJMIOHKANIA[(num3 >> 24) & 0x3F];
			num ^= num4;
			num3 = (num << 28) | (num >> 4);
			num3 ^= (uint)IIIGKBPIKGE[i * 4 + 2];
			num4 = ONOJPLONDMO[num3 & 0x3F];
			num4 |= FNOFEPDINLN[(num3 >> 8) & 0x3F];
			num4 |= OPHIFECEKJM[(num3 >> 16) & 0x3F];
			num4 |= HLKNLALDKBA[(num3 >> 24) & 0x3F];
			num3 = num ^ (uint)IIIGKBPIKGE[i * 4 + 3];
			num4 |= NDKJBHKFPPG[num3 & 0x3F];
			num4 |= GAMFPABJNDC[(num3 >> 8) & 0x3F];
			num4 |= CLJNBBNMKOC[(num3 >> 16) & 0x3F];
			num4 |= EJMIOHKANIA[(num3 >> 24) & 0x3F];
		...
		num ^= num3;
		num2 ^= num3 << 4;
		PGOKFBKEOAB.MPMAFPANDLC(num2, NGAGMIIBGKA, EMAKNGBIFDC);
		PGOKFBKEOAB.MPMAFPANDLC(num, NGAGMIIBGKA, EMAKNGBIFDC + 4);
	}
}
