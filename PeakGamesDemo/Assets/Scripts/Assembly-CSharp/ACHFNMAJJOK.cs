...
public class ACHFNMAJJOK : JMPFLJGHAIM
{
	...
	private static readonly byte[] OBMHBABLEPB = new byte[256]
	{
	...
	private static readonly byte[] EMPNNLHEDJC = new byte[30]
	{
	...
	private static readonly uint[] ICPIAMJKOCA = new uint[256]
	{
	...
	private static readonly uint[] LMAEDOHGLAD = new uint[256]
	{
	...
	private const uint BCLCBBPJING = 2155905152u;

	private const uint KNNMFKKIPDJ = 2139062143u;

	private const uint DBHHEBLNOHB = 27u;

	private const uint MAJAJOOBEMP = 3233857728u;

	private const uint NHJABMPNKJE = 1061109567u;

	private int DFBJOFIADHK;

	private uint[][] JFGNONNOEOP;

	private uint DMGCLBGPJEC;

	private uint CIDLKLHOKHP;

	private uint PLJEFFLCHFO;

	private uint IMBAIFONFBO;
	...
	private static uint FMLAJIGEAKE(uint GPGDCCMECGF, int IMLCPJDGOEO)
	{
		return (GPGDCCMECGF >> IMLCPJDGOEO) | (GPGDCCMECGF << 32 - IMLCPJDGOEO);
	}

	private static uint ICGMLICMCFH(uint AMFCENFNAJJ)
	{
		return ((AMFCENFNAJJ & 0x7F7F7F7F) << 1) ^ (((AMFCENFNAJJ & 0x80808080u) >> 7) * 27);
	}

	private static uint HHDOGELHEBJ(uint AMFCENFNAJJ)
	{
		uint num = (AMFCENFNAJJ & 0x3F3F3F3F) << 2;
		uint num2 = AMFCENFNAJJ & 0xC0C0C0C0u;
		num2 ^= num2 >> 1;
		return num ^ (num2 >> 2) ^ (num2 >> 5);
	}

	private static uint CCFDACBBOFH(uint AMFCENFNAJJ)
	{
		uint num = AMFCENFNAJJ;
		uint num2 = num ^ FMLAJIGEAKE(num, 8);
		num ^= ICGMLICMCFH(num2);
		num2 ^= HHDOGELHEBJ(num);
		return num ^ (num2 ^ FMLAJIGEAKE(num2, 16));
	}

	private static uint KLMKAKIFLKD(uint AMFCENFNAJJ)
	{
		return (uint)(PCPPGMNCBJG[AMFCENFNAJJ & 0xFF] | (PCPPGMNCBJG[(AMFCENFNAJJ >> 8) & 0xFF] << 8) | (PCPPGMNCBJG[(AMFCENFNAJJ >> 16) & 0xFF] << 16) | (PCPPGMNCBJG[(AMFCENFNAJJ >> 24) & 0xFF] << 24));
	}

	private uint[][] DNIOCLEEKMH(byte[] IKNCPEPOKGJ, bool MAEALKEJALK)
	{
		...
		int num2 = num >> 2;
		DFBJOFIADHK = num2 + 6;
		uint[][] array = new uint[DFBJOFIADHK + 1][];
		for (int i = 0; i <= DFBJOFIADHK; i++)
		{
			array[i] = new uint[4];
		}
		switch (num2)
		...
		case 4:
		{
			uint num21 = PGOKFBKEOAB.LAJPMLKCDOH(IKNCPEPOKGJ, 0);
			array[0][0] = num21;
			uint num22 = PGOKFBKEOAB.LAJPMLKCDOH(IKNCPEPOKGJ, 4);
			array[0][1] = num22;
			uint num23 = PGOKFBKEOAB.LAJPMLKCDOH(IKNCPEPOKGJ, 8);
			array[0][2] = num23;
			uint num24 = PGOKFBKEOAB.LAJPMLKCDOH(IKNCPEPOKGJ, 12);
			array[0][3] = num24;
			for (int l = 1; l <= 10; l++)
			{
				uint num25 = KLMKAKIFLKD(FMLAJIGEAKE(num24, 8)) ^ EMPNNLHEDJC[l - 1];
				num21 ^= num25;
				array[l][0] = num21;
				num22 ^= num21;
				array[l][1] = num22;
		...
		case 6:
		{
			uint num13 = PGOKFBKEOAB.LAJPMLKCDOH(IKNCPEPOKGJ, 0);
			array[0][0] = num13;
			uint num14 = PGOKFBKEOAB.LAJPMLKCDOH(IKNCPEPOKGJ, 4);
			array[0][1] = num14;
			uint num15 = PGOKFBKEOAB.LAJPMLKCDOH(IKNCPEPOKGJ, 8);
			array[0][2] = num15;
			uint num16 = PGOKFBKEOAB.LAJPMLKCDOH(IKNCPEPOKGJ, 12);
			array[0][3] = num16;
			uint num17 = PGOKFBKEOAB.LAJPMLKCDOH(IKNCPEPOKGJ, 16);
			array[1][0] = num17;
			uint num18 = PGOKFBKEOAB.LAJPMLKCDOH(IKNCPEPOKGJ, 20);
			array[1][1] = num18;
			uint num19 = 1u;
			uint num20 = KLMKAKIFLKD(FMLAJIGEAKE(num18, 8)) ^ num19;
			num19 <<= 1;
			num13 ^= num20;
			array[1][2] = num13;
			num14 ^= num13;
			...
			{
				num20 = KLMKAKIFLKD(FMLAJIGEAKE(num18, 8)) ^ num19;
				num19 <<= 1;
				num13 ^= num20;
				array[k][0] = num13;
				num14 ^= num13;
		...
		case 8:
		{
			uint num3 = PGOKFBKEOAB.LAJPMLKCDOH(IKNCPEPOKGJ, 0);
			array[0][0] = num3;
			uint num4 = PGOKFBKEOAB.LAJPMLKCDOH(IKNCPEPOKGJ, 4);
			array[0][1] = num4;
			uint num5 = PGOKFBKEOAB.LAJPMLKCDOH(IKNCPEPOKGJ, 8);
			array[0][2] = num5;
			uint num6 = PGOKFBKEOAB.LAJPMLKCDOH(IKNCPEPOKGJ, 12);
			array[0][3] = num6;
			uint num7 = PGOKFBKEOAB.LAJPMLKCDOH(IKNCPEPOKGJ, 16);
			array[1][0] = num7;
			uint num8 = PGOKFBKEOAB.LAJPMLKCDOH(IKNCPEPOKGJ, 20);
			array[1][1] = num8;
			uint num9 = PGOKFBKEOAB.LAJPMLKCDOH(IKNCPEPOKGJ, 24);
			array[1][2] = num9;
			uint num10 = PGOKFBKEOAB.LAJPMLKCDOH(IKNCPEPOKGJ, 28);
			array[1][3] = num10;
			uint num11 = 1u;
			uint num12;
			for (int j = 2; j < 14; j += 2)
			{
				num12 = KLMKAKIFLKD(FMLAJIGEAKE(num10, 8)) ^ num11;
				num11 <<= 1;
				num3 ^= num12;
				array[j][0] = num3;
				num4 ^= num3;
		...
		if (!MAEALKEJALK)
		{
			for (int m = 1; m < DFBJOFIADHK; m++)
			{
				uint[] array2 = array[m];
				for (int n = 0; n < 4; n++)
				{
					array2[n] = CCFDACBBOFH(array2[n]);
				}
			}
		}
		return array;
	...
	public virtual void DIOELAHNLKJ(bool MAEALKEJALK, IGMAJHKIIPK IJJHBEECMBP)
	{
		...
		if (jEMNCGMEABF == null)
		{
			throw new ArgumentException("invalid parameter passed to AES init - " + LHFGHNFJIKM.JKHOEKAOLPM(IJJHBEECMBP));
		}
		JFGNONNOEOP = DNIOCLEEKMH(jEMNCGMEABF.MEGBFHFMBFI(), MAEALKEJALK);
	...
	public virtual int DPACJDFHLKB(byte[] MOPCLEEGDFB, int ELICNEDIBGB, byte[] BILGCJNPJGJ, int EMAKNGBIFDC)
	{
		...
		ONDCOJJCHGD(MOPCLEEGDFB, ELICNEDIBGB);
		if (MAEALKEJALK)
		{
			HDOLNBOHAJA(JFGNONNOEOP);
		}
		else
		{
			GFKALHGNIOJ(JFGNONNOEOP);
		}
		KKAFNAFKNHF(BILGCJNPJGJ, EMAKNGBIFDC);
		return 16;
	}
	...
	private void ONDCOJJCHGD(byte[] DIPNEDDIHBK, int JBAJPGIAPFF)
	{
		DMGCLBGPJEC = PGOKFBKEOAB.LAJPMLKCDOH(DIPNEDDIHBK, JBAJPGIAPFF);
		CIDLKLHOKHP = PGOKFBKEOAB.LAJPMLKCDOH(DIPNEDDIHBK, JBAJPGIAPFF + 4);
		PLJEFFLCHFO = PGOKFBKEOAB.LAJPMLKCDOH(DIPNEDDIHBK, JBAJPGIAPFF + 8);
		IMBAIFONFBO = PGOKFBKEOAB.LAJPMLKCDOH(DIPNEDDIHBK, JBAJPGIAPFF + 12);
	}

	private void KKAFNAFKNHF(byte[] DIPNEDDIHBK, int JBAJPGIAPFF)
	{
		PGOKFBKEOAB.LGJFKMLLKLN(DMGCLBGPJEC, DIPNEDDIHBK, JBAJPGIAPFF);
		PGOKFBKEOAB.LGJFKMLLKLN(CIDLKLHOKHP, DIPNEDDIHBK, JBAJPGIAPFF + 4);
		PGOKFBKEOAB.LGJFKMLLKLN(PLJEFFLCHFO, DIPNEDDIHBK, JBAJPGIAPFF + 8);
		PGOKFBKEOAB.LGJFKMLLKLN(IMBAIFONFBO, DIPNEDDIHBK, JBAJPGIAPFF + 12);
	}

	private void HDOLNBOHAJA(uint[][] KOGHJMFFKLL)
	{
		uint[] array = KOGHJMFFKLL[0];
		uint num = DMGCLBGPJEC ^ array[0];
		uint num2 = CIDLKLHOKHP ^ array[1];
		uint num3 = PLJEFFLCHFO ^ array[2];
		uint num4 = IMBAIFONFBO ^ array[3];
		int num5 = 1;
		uint num6;
		uint num7;
		uint num8;
		while (num5 < DFBJOFIADHK - 1)
		{
			array = KOGHJMFFKLL[num5++];
			num6 = ICPIAMJKOCA[num & 0xFF] ^ FMLAJIGEAKE(ICPIAMJKOCA[(num2 >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num3 >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num4 >> 24) & 0xFF], 8) ^ array[0];
			num7 = ICPIAMJKOCA[num2 & 0xFF] ^ FMLAJIGEAKE(ICPIAMJKOCA[(num3 >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num4 >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num >> 24) & 0xFF], 8) ^ array[1];
			num8 = ICPIAMJKOCA[num3 & 0xFF] ^ FMLAJIGEAKE(ICPIAMJKOCA[(num4 >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num2 >> 24) & 0xFF], 8) ^ array[2];
			num4 = ICPIAMJKOCA[num4 & 0xFF] ^ FMLAJIGEAKE(ICPIAMJKOCA[(num >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num2 >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num3 >> 24) & 0xFF], 8) ^ array[3];
			array = KOGHJMFFKLL[num5++];
			num = ICPIAMJKOCA[num6 & 0xFF] ^ FMLAJIGEAKE(ICPIAMJKOCA[(num7 >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num8 >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num4 >> 24) & 0xFF], 8) ^ array[0];
			num2 = ICPIAMJKOCA[num7 & 0xFF] ^ FMLAJIGEAKE(ICPIAMJKOCA[(num8 >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num4 >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num6 >> 24) & 0xFF], 8) ^ array[1];
			num3 = ICPIAMJKOCA[num8 & 0xFF] ^ FMLAJIGEAKE(ICPIAMJKOCA[(num4 >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num6 >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num7 >> 24) & 0xFF], 8) ^ array[2];
			num4 = ICPIAMJKOCA[num4 & 0xFF] ^ FMLAJIGEAKE(ICPIAMJKOCA[(num6 >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num7 >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num8 >> 24) & 0xFF], 8) ^ array[3];
		}
		array = KOGHJMFFKLL[num5++];
		num6 = ICPIAMJKOCA[num & 0xFF] ^ FMLAJIGEAKE(ICPIAMJKOCA[(num2 >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num3 >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num4 >> 24) & 0xFF], 8) ^ array[0];
		num7 = ICPIAMJKOCA[num2 & 0xFF] ^ FMLAJIGEAKE(ICPIAMJKOCA[(num3 >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num4 >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num >> 24) & 0xFF], 8) ^ array[1];
		...
		num4 = ICPIAMJKOCA[num4 & 0xFF] ^ FMLAJIGEAKE(ICPIAMJKOCA[(num >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num2 >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(ICPIAMJKOCA[(num3 >> 24) & 0xFF], 8) ^ array[3];
		array = KOGHJMFFKLL[num5];
		DMGCLBGPJEC = (uint)(PCPPGMNCBJG[num6 & 0xFF] ^ (PCPPGMNCBJG[(num7 >> 8) & 0xFF] << 8) ^ (PCPPGMNCBJG[(num8 >> 16) & 0xFF] << 16) ^ (PCPPGMNCBJG[(num4 >> 24) & 0xFF] << 24)) ^ array[0];
		CIDLKLHOKHP = (uint)(PCPPGMNCBJG[num7 & 0xFF] ^ (PCPPGMNCBJG[(num8 >> 8) & 0xFF] << 8) ^ (PCPPGMNCBJG[(num4 >> 16) & 0xFF] << 16) ^ (PCPPGMNCBJG[(num6 >> 24) & 0xFF] << 24)) ^ array[1];
		PLJEFFLCHFO = (uint)(PCPPGMNCBJG[num8 & 0xFF] ^ (PCPPGMNCBJG[(num4 >> 8) & 0xFF] << 8) ^ (PCPPGMNCBJG[(num6 >> 16) & 0xFF] << 16) ^ (PCPPGMNCBJG[(num7 >> 24) & 0xFF] << 24)) ^ array[2];
		IMBAIFONFBO = (uint)(PCPPGMNCBJG[num4 & 0xFF] ^ (PCPPGMNCBJG[(num6 >> 8) & 0xFF] << 8) ^ (PCPPGMNCBJG[(num7 >> 16) & 0xFF] << 16) ^ (PCPPGMNCBJG[(num8 >> 24) & 0xFF] << 24)) ^ array[3];
	}

	private void GFKALHGNIOJ(uint[][] KOGHJMFFKLL)
	{
		uint[] array = KOGHJMFFKLL[DFBJOFIADHK];
		uint num = DMGCLBGPJEC ^ array[0];
		uint num2 = CIDLKLHOKHP ^ array[1];
		uint num3 = PLJEFFLCHFO ^ array[2];
		uint num4 = IMBAIFONFBO ^ array[3];
		int num5 = DFBJOFIADHK - 1;
		uint num6;
		uint num7;
		uint num8;
		while (num5 > 1)
		{
			array = KOGHJMFFKLL[num5--];
			num6 = LMAEDOHGLAD[num & 0xFF] ^ FMLAJIGEAKE(LMAEDOHGLAD[(num4 >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num3 >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num2 >> 24) & 0xFF], 8) ^ array[0];
			num7 = LMAEDOHGLAD[num2 & 0xFF] ^ FMLAJIGEAKE(LMAEDOHGLAD[(num >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num4 >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num3 >> 24) & 0xFF], 8) ^ array[1];
			num8 = LMAEDOHGLAD[num3 & 0xFF] ^ FMLAJIGEAKE(LMAEDOHGLAD[(num2 >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num4 >> 24) & 0xFF], 8) ^ array[2];
			num4 = LMAEDOHGLAD[num4 & 0xFF] ^ FMLAJIGEAKE(LMAEDOHGLAD[(num3 >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num2 >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num >> 24) & 0xFF], 8) ^ array[3];
			array = KOGHJMFFKLL[num5--];
			num = LMAEDOHGLAD[num6 & 0xFF] ^ FMLAJIGEAKE(LMAEDOHGLAD[(num4 >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num8 >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num7 >> 24) & 0xFF], 8) ^ array[0];
			num2 = LMAEDOHGLAD[num7 & 0xFF] ^ FMLAJIGEAKE(LMAEDOHGLAD[(num6 >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num4 >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num8 >> 24) & 0xFF], 8) ^ array[1];
			num3 = LMAEDOHGLAD[num8 & 0xFF] ^ FMLAJIGEAKE(LMAEDOHGLAD[(num7 >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num6 >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num4 >> 24) & 0xFF], 8) ^ array[2];
			num4 = LMAEDOHGLAD[num4 & 0xFF] ^ FMLAJIGEAKE(LMAEDOHGLAD[(num8 >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num7 >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num6 >> 24) & 0xFF], 8) ^ array[3];
		}
		array = KOGHJMFFKLL[1];
		num6 = LMAEDOHGLAD[num & 0xFF] ^ FMLAJIGEAKE(LMAEDOHGLAD[(num4 >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num3 >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num2 >> 24) & 0xFF], 8) ^ array[0];
		num7 = LMAEDOHGLAD[num2 & 0xFF] ^ FMLAJIGEAKE(LMAEDOHGLAD[(num >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num4 >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num3 >> 24) & 0xFF], 8) ^ array[1];
		...
		num4 = LMAEDOHGLAD[num4 & 0xFF] ^ FMLAJIGEAKE(LMAEDOHGLAD[(num3 >> 8) & 0xFF], 24) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num2 >> 16) & 0xFF], 16) ^ FMLAJIGEAKE(LMAEDOHGLAD[(num >> 24) & 0xFF], 8) ^ array[3];
		array = KOGHJMFFKLL[0];
		DMGCLBGPJEC = (uint)(OBMHBABLEPB[num6 & 0xFF] ^ (OBMHBABLEPB[(num4 >> 8) & 0xFF] << 8) ^ (OBMHBABLEPB[(num8 >> 16) & 0xFF] << 16) ^ (OBMHBABLEPB[(num7 >> 24) & 0xFF] << 24)) ^ array[0];
		CIDLKLHOKHP = (uint)(OBMHBABLEPB[num7 & 0xFF] ^ (OBMHBABLEPB[(num6 >> 8) & 0xFF] << 8) ^ (OBMHBABLEPB[(num4 >> 16) & 0xFF] << 16) ^ (OBMHBABLEPB[(num8 >> 24) & 0xFF] << 24)) ^ array[1];
		PLJEFFLCHFO = (uint)(OBMHBABLEPB[num8 & 0xFF] ^ (OBMHBABLEPB[(num7 >> 8) & 0xFF] << 8) ^ (OBMHBABLEPB[(num6 >> 16) & 0xFF] << 16) ^ (OBMHBABLEPB[(num4 >> 24) & 0xFF] << 24)) ^ array[2];
		IMBAIFONFBO = (uint)(OBMHBABLEPB[num4 & 0xFF] ^ (OBMHBABLEPB[(num8 >> 8) & 0xFF] << 8) ^ (OBMHBABLEPB[(num7 >> 16) & 0xFF] << 16) ^ (OBMHBABLEPB[(num6 >> 24) & 0xFF] << 24)) ^ array[3];
	}
}
